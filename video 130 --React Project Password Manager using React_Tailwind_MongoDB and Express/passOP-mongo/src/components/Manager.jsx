...
const Manager = () => {
  ...
  const savePassword = async () => {
    if (form.site.length > 3 && form.username.length > 3 && form.password.length > 3) {
      ...
      toast.error('Error: Please fill all fields properly!', {
        autoClose: 3000,
        theme: "dark",
        transition: Bounce,
      })
  ...
  // Edit password (loads data back into form)
  const editPassword = (id) => {
    const record = passwordArray.find(i => i.id === id)
    if (record) {
      setForm(record)
      setPasswordArray(passwordArray.filter(item => item.id !== id))
    }
  ...
  return (
    <>
      ...
      <div className="absolute inset-0 -z-10 h-full w-full bg-green-50 
        ...
        bg-[size:14px_24px]">
        <div className="absolute left-0 right-0 top-0 -z-10 m-auto h-[310px] 
          w-[310px] rounded-full bg-green-400 opacity-20 blur-[100px]"></div>
      </div>
        ...
        <div className="flex flex-col items-center p-4 text-black gap-8">
          ...
          <div className="flex flex-col md:flex-row w-full gap-8">
            ...
            <div className="relative">
              ...
              <span className="absolute right-2.5 self-center cursor-pointer" onClick={showPassword}>
                <img ref={ref} className='w-[27px] p-1' src="icons/eye.png" alt="toggle" />
              </span>
            </div>
          </div>
